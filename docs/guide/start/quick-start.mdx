...
If the spec was generated successfully, you should see the json file in the root of the project. Time to integrate the frontend!

## Frontend

Time to build the frontend application. Change into the `ui` directory if you haven't already:
...
> [!DETAILS]
> This demo uses `Vite`, `React 19`, and `React Router`. If you don't know how to setup a project with these tools, the source code of this demo is provided at the end of this guide.
...
If using a plugin configuration file, run the command with the configuration option:
...
Stately uses tailwindcss under the hood. Create `src/index.css` and configure it to pull in Stately's styles:
...
All of the source code for this simple quick-start are available in the stately demos folder, this quick start can be found in the [basic demo](https://github.com/GeorgeLeePatterson/stately/tree/main/demos/basic).

## Next Steps

To see a similar demo with a bit more concepts expanded, review the [Tasks demo](../demos/basic.md). To learn a bit more about why stately was built in the way it was, head over to [`Why Stately`](./why-stately.md).
