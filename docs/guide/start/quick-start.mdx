...
> [!INFO]
> In the previous file, `@statelyjs/ui` components were imported using `@statelyjs/stately/ui`. For simple use cases, stately re-exports the most commonly used components from 'ui'. If you want access to the base components or other items not exported by stately, you can install `@statelyjs/ui` directly and import them yourself.

### Styles and Tailwind

Stately uses tailwindcss under the hood. Create `src/index.css` and pull in Stately's styles.

> [!INFO]
> If your app is *not* using Tailwind, then this is sufficient. Otherwise, refer to [Tailwind's documentation](https://tailwindcss.com/docs) on how to configure your project. All of Stately's styles have been compiled already, so all you need to do is import them, unless the goal is to customize the styles yourself or overwrite some of the tokens used:

```css
/* Stately framework styles (includes ui tokens/global + all utilities) */
