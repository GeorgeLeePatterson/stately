...
function transformSchema(schema: any, ctx: CodegenPluginContext) {
  ...
  if (schema.type === 'array') {
    if (schema.prefixItems) {
      ...
      if (items.length === schema.prefixItems.length) {
        return { nodeType: CoreNodeType.Tuple, items, description: schema.description };
      }
    }

    const itemSchema = schema.items ? ctx.parseSchema(schema.items, ctx.schemaName) : null;
    return { nodeType: CoreNodeType.Array, items: itemSchema, description: schema.description };
  }
  ...
  if (schema.type === 'object' && schema.additionalProperties && !schema.properties) {
    ...
    return { nodeType: CoreNodeType.Map, valueSchema, description: schema.description };
  }
  ...
  if (
    ...
    if (schema.enum && schema.enum.length > 0) {
      return { nodeType: CoreNodeType.Enum, values: schema.enum, description: schema.description };
    }
...
function detectLinkNode(schema: any, ctx: CodegenPluginContext) {
  ...
  const bothHaveEntityType = variants.every((v: any) => {
    ...
    return resolved?.properties?.entity_type?.enum && Array.isArray(resolved.properties.entity_type.enum);
  });
  ...
  for (const variant of schema.oneOf) {
    ...
    if (resolved.type === 'object') {
      ...
      if (isUntaggedEnum) {
        ...
        const variantSchema = parsed || { nodeType: CoreNodeType.Object, properties: {}, required: [] };
