...
export function createCoreCodegenPlugin(): CodegenPlugin {
  return {
    /**
     * Extract entry points from the Entity schema.
     * Core's entities are defined as a oneOf in the Entity schema, where each variant
     * has a `data` property with a $ref to the actual entity schema.
     *
     * Example Entity schema structure:
     * ```json
     * {
     *   "oneOf": [
     *     { "properties": { "data": { "$ref": "#/components/schemas/Pipeline" }, "type": { "enum": ["pipeline"] } } },
     *     { "properties": { "data": { "$ref": "#/components/schemas/CollectionConfig" }, "type": { "enum": ["collection_configs"] } } }
     *   ]
     * }
     * ```
     */
    entryPoints(spec: OpenAPISpec): string[] | undefined {
      // biome-ignore lint/complexity/useLiteralKeys: Must be accessor
      const entitySchema = spec.components?.schemas?.['Entity'];
      if (!entitySchema?.oneOf) {
        // No Entity schema or not a oneOf - fall back to bundling all
        return undefined;
      }

      const entryPoints: string[] = [];

      for (const variant of entitySchema.oneOf) {
        // Extract $ref from data property
        const dataRef = variant?.properties?.data?.$ref;
        if (dataRef && typeof dataRef === 'string') {
          const schemaName = dataRef.split('/').pop();
          if (schemaName) {
            entryPoints.push(schemaName);
          }
        }
      }
      ...
      // Also include StateEntry as they're core to the system
      entryPoints.push(/** 'Entity', */ 'StateEntry');
