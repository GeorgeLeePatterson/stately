...
export type ArrayEditProps<Config extends StatelyConfig = StatelyConfig> = EditFieldProps<
  Config,
 ...
 */
export function ArrayEdit<Config extends StatelyConfig = StatelyConfig>({
  label,
  node,
  value,
  onChange,
  ...
  const getItemLabel = (index: number, item: any) => {
    if (itemNode.nodeType === NodeType.Link) {
      const targetName = integration.entityDisplayNames?.[itemNode.targetType] || 'Item';
  ...
  const handleAdd = () => {
    const defaultValue = integration.helpers.getDefaultValue(node);
