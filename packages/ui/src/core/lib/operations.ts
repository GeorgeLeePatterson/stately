...
export type OperationOverrides<Schema extends CoreSchemas = CoreSchemas> =
  Partial<{
...
>(
  ...
  const entries = Object.entries(DEFAULT_OPERATION_IDS).map(
    ([key, operationId]) => {
...
export interface StatelyEntityApi<Schema extends CoreSchemas = CoreSchemas> {
  listByType(args: { type: CoreStateEntry<Schema> }): Promise<any>;
  get(args: { id: string; type: CoreStateEntry<Schema> }): Promise<any>;
}
