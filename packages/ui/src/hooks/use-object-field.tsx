...
export const useObjectField = ({
  ...
  const fields = integration.helpers.sortEntityProperties(valueFields, value, required);

  const isValid =
    integration?.validate?.object(`${label ? label : ''}[ObjectNode]`, formData, node)?.valid ||
    true;
